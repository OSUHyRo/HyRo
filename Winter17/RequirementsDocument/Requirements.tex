...
\index{Message and Errors Input/Output}
	The system status input receives error and command response data from the radio transceiver to be passed to the user interface. The status will be formatted with the item name appended by the status data from that part of the system. Status messages will be delivered in under 1 second. 
...
\paragraph{\bf Linux OS Interface}This interface will need to connect to the operating systems UART lines to communicate with the XBee. It will also be running in the Linux environment and follow rules pertaining to that environment.
...
\index{Sensor Input}
	...
	\paragraph{ Requirement 3}Sensor data will be gathered in a loop and packed into a packet to be sent to the radio transceiver.\par
